 ...
 * Evaluates text using 15 modular heuristic detectors.
 *
 * @param {string} text - The content text to analyze
 * @param {Object} options - Configuration overrides
 * @param {Object} options.rules - Custom rules and weights mapping
 * @param {Array<string>} options.keywords - Custom promotional keyword list
 * @param {Array<string>} options.blacklist - Custom blacklisted domains
 * @param {Array<string>} options.whitelist - Whitelisted domains
 * @param {Array<string>} options.recentContent - List of recently submitted texts for duplicate detection
...
export function analyzeContentSpam(text, options = {}) {
  ...
  const addDetectorReport = (id, result) => {
    ...
    if (result.triggered) {
      ...
      // Detector score is proportional to the trigger strength up to the full weight
  ...
  (() => {
    ...
    if (isTriggered) {
      const factor = Math.min((maxConsecutive - 4) / 5, 1.0) // 1.0 at 9+ repetitions
  ...
  (() => {
    ...
    if (urls.length > 0) {
      // Calculate URL density (length of URLs relative to text length)
  ...
  (() => {
    ...
    keywords.forEach(keyword => {
      ...
      // Support exact word boundaries and fuzzy checks
  ...
  (() => {
    // Matches Indian (+91), US (+1), and general international formats
  ...
  (() => {
    if (content.length >= 10) {
      ...
      // Keyboard sequences (rows of QWERTY)
      const seqPattern = /asdf|asda|qwer|zxcv|qweq|zxcz|sdfg/gi
      const seqMatches = content.match(seqPattern) || []
      
      // Gibberish often has high entropy but no spaces, or low entropy with repeating keys
      const words = content.split(/\s+/)
  ...
  (() => {
    if (content.length >= 10) {
      // Find all special characters (excluding letters, digits, and spaces)
      const specialChars = content.replace(/[a-zA-Z0-9\s]/g, '')
      const ratio = specialChars.length / content.length
      
      // Also look for bursts of consecutive special characters (e.g. "!!!!!!!!!", "$$$$$$$")
